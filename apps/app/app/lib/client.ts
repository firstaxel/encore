...
export function Environment(name: string): BaseURL {
  return `https://${name}-apiw-7t52.encr.app`;
}
...
/**
 * Client is an API client for the apiw-7t52 Encore application.
 */
...
export namespace profile {
  ...
  export class ServiceClient {
    ...
    public async count(): Promise<ProfileResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI('GET', `/count/profiles`);
      return (await resp.json()) as ProfileResponse;
    }

    public async create(params: UserProfile): Promise<ProfileResponse> {
      // Now make the actual call to the API
  ...
  constructor(baseURL: string, options: ClientOptions) {
    ...
    if (typeof window === 'undefined') {
      this.headers['User-Agent'] =
        'apiw-7t52-Generated-TS-Client (Encore/v1.46.9)';
    }
